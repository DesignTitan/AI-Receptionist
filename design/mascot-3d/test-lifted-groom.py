"""Unapproved groom experiment: renders a forehead swatch only. No master replacement.
...
import os
os.environ["MASCOT_SWATCH"]="1"  # This experiment must never replace the full character.
...
for i in range(16000):
 p,n=sample();d=flow(p,n);length=.062*(1+.08*math.sin(p.x*3+p.z*2))
 guides.append((p,n,d,length));tree.insert(p,i)
tree.balance()
COUNT=650000;STEPS=9
...
for i in range(COUNT):
 ...
 d=flow(p,n);length=gl*random.uniform(.94,1.06);undercoat=random.random()<.42
 if undercoat:length*=.46
 jitter=random.uniform(-1,1);side=n.cross(d).normalized();width=random.uniform(.00065,.00090) if not undercoat else random.uniform(.00085,.0011)
 for k in range(STEPS):
  t=k/(STEPS-1)
  own=p+n*length*(.95*t-.18*t*t)+d*length*(.40*t*t)
  guide=gp+gn*gl*(.95*t-.18*t*t)+gd*gl*.40*t*t
  # Gentle convergence: tiny layered tufts, not wet spikes or uniform grass.
  pos=own.lerp(guide,.16*t*t if not undercoat else 0)
  pos+=side*math.sin(t*math.pi*1.4+jitter)*length*.025*t
  positions[i*STEPS+k]=pos
  radii[i*STEPS+k]=width*(1-t)**.75+.000015
curves=bpy.data.hair_curves.new('Native hair • 650,000 layered fibres');curves.add_curves([STEPS]*COUNT)
...
if os.environ.get('MASCOT_SWATCH')=='1':
 scene.camera.data.ortho_scale=.65;scene.camera.location=(0,-8,2.10)
 scene.camera.rotation_euler=(Vector((0,0,2.08))-scene.camera.location).to_track_quat('-Z','Y').to_euler()
 scene.render.resolution_x=650;scene.render.resolution_y=650;scene.cycles.samples=512;scene.cycles.adaptive_threshold=.003
...
if os.environ.get('MASCOT_SWATCH')=='1':
 scene.render.filepath=str(RENDERS/'audit/lifted-groom.png')
else:bpy.ops.wm.save_as_mainfile(filepath=str(BASE/'mascot-film.blend'),compress=True)
