...
import { validateConfig, type Customer, type Plan } from "@/lib/platform/model";
...
const zoneLabels:Record<string,string>={"America/New_York":"Eastern Time (US & Canada)","America/Detroit":"Eastern Time (Michigan)","America/Chicago":"Central Time (US & Canada)","America/Denver":"Mountain Time (US & Canada)","America/Los_Angeles":"Pacific Time (US & Canada)","America/Phoenix":"Arizona Time (no daylight saving)","America/Anchorage":"Alaska Time","Pacific/Honolulu":"Hawaii Time (no daylight saving)"};
...
function SetupFields({customer,preview=false,initialStep=1,embedded=false,draft}:{draft?:SetupDraft}&SetupProps) {
 const c=customer?.config.setupPending?undefined:customer?.config;
 const [step,setStep]=useState(initialStep),[team,setTeam]=useState(draft?.team??c?.team??(preview?[{id:"member-1",name:"Willow Studio",service:"Appointment",minutes:45}]:[{id:"member-1",name:"",service:"",minutes:30}]));
 ...
 useEffect(()=>{
  const area=form.current?.elements.namedItem("areaCode") as HTMLInputElement|null;
  const initialPhone=(readDetails().phone??"").replace(/\D/g,"").replace(/^1(?=\d{10}$)/,"");
  if(area&&!area.value&&/^[2-9]\d{9}$/.test(initialPhone))area.value=initialPhone.slice(0,3);
 ...
 const config=(d:Record<string,string>)=>({answeringPreference:d.answeringPreference,trade:d.trade,customTrade:d.customTrade,address:d.address,phone:d.phone,areaCode:d.areaCode,color:d.color,weeklyHours,days,opens,closes,timezone,team:[{id:"member-1",name:d.business_name,service:"Appointment",minutes:team[0]?.minutes??30}],phoneSetup:{provider:d.phoneProvider,serviceType:d.phoneServiceType,serviceName:d.phoneServiceName,bookingSystem:d.bookingSystem}});
 async function submit(e:React.FormEvent){e.preventDefault();if(busy)return;setError("");setSaved(false);
  ...
  for(const input of inputs){if(!input.checkValidity()){move(Number(input.closest("[data-step]")?.getAttribute("data-step")??1));input.scrollIntoView({behavior:"instant",block:"center"});input.focus({preventScroll:true});input.reportValidity();return;}}
  const d=readDetails();setDetails(d);
  try{validateConfig(config(d));setBusy(true);if(preview||customer?.config.setupPending)await autosave.flush();if(preview){setSaved(true);return;}const r=await fetch("/api/account/business",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({business_name:d.business_name,config:config(d)})});const data=await r.json();if(!r.ok)throw Error(data.error??"We couldn’t save your setup.");location.assign("/account");}catch(e){setError(e instanceof Error?e.message:"Please check your details.");}finally{setBusy(false);}}
 ...
 const content=<>
  ...
  <form ref={form} onChange={event=>{const input=event.target as unknown as HTMLInputElement;if(input.name==="areaCode")areaCodeChosen.current=Boolean(input.value);if(input.name==="phone"&&!areaCodeChosen.current){const digits=input.value.replace(/\D/g,"").replace(/^1(?=\d{10}$)/,"");const area=form.current?.elements.namedItem("areaCode") as HTMLInputElement|null;if(area&&/^[2-9]\d{9}$/.test(digits))area.value=digits.slice(0,3);}setDetails(readDetails());setSaved(false);setError("");}} onSubmit={submit} noValidate className={styles.form}>
   ...
   <section id="review-setup" tabIndex={-1} aria-labelledby="review-title" data-step="3" className={styles.section}><h2 id="review-title" className={styles.sectionTitle}>Review & setup</h2><div className={styles.columns}>
    ...
    <aside className={`${styles.card} ${styles.next}`}><Image src="/marketing/happy-mascot-pointed.png" alt="" width={96} height={96} sizes="96px"/><h2>What happens next</h2><p>{preview?"For a real account, these steps begin after you submit your setup.":"Here’s what we’ll do after you send this."}</p><ol>{[["We prepare your front desk","Your booking page and phone setup."],["We test it together","We’ll contact you to arrange a test."],["You’re ready to welcome bookings","We’ll email you when it’s live."]].map(([h,p],i)=><li key={h}><span>{i+1}</span><div><h3>{h}</h3><p>{p}</p></div></li>)}</ol><p className={styles.callout}>Need to change something? You can edit before sending.</p></aside>
   </div>
   {error&&<p role="alert" className={styles.error}>{error}</p>}
   {saved&&<div role="status" className={styles.callout}><strong>Preview complete.</strong><p>Your details are saved in this browser. Nothing was sent to our team because this is a practice account.</p><Link href="/account/login">Sign in to submit your real business setup →</Link></div>}
   <div className={styles.actions}><Link href={preview?"/account?preview=confirmation":"/account"} className={styles.secondary}>Back to overview</Link><div><button className={styles.primary} disabled={busy||(preview&&saved)}>{busy?(preview?"Saving…":"Sending…"):preview?(saved?"Preview complete":"Finish preview →"):"Send for setup →"}</button><p>{preview?"Practice account · Saved on this device only. Nothing will be sent.":"Your service is not live until setup and testing are complete."}</p></div></div>
   </section>
  </form>
 </>;
