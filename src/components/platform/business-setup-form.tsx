...
function SetupFields({customer,plan,preview=false,initialStep=1,embedded=false,draft}:{draft?:SetupDraft}&SetupProps) {
 ...
 const [days,setDays]=useState(draft?.days??c?.days??[1,2,3,4,5]),[opens,setOpens]=useState(draft?.opens??c?.opens??"09:00"),[closes,setCloses]=useState(draft?.closes??c?.closes??"17:00"),[timezone,setTimezone]=useState(draft?.timezone??c?.timezone??"America/New_York");
 ...
 const autosave=useSetupAutosave({details,days,opens,closes,timezone,team},preview,!busy&&(preview||!!customer?.config.setupPending));
 ...
 const config=(d:Record<string,string>)=>({answeringPreference:d.answeringPreference,trade:d.trade,address:d.address,phone:d.phone,areaCode:d.areaCode,color:d.color,days,opens,closes,timezone,team:[{id:"member-1",name:d.business_name,service:"Appointment",minutes:team[0]?.minutes??30}],phoneSetup:{provider:d.phoneProvider,serviceType:d.phoneServiceType,serviceName:d.phoneServiceName,bookingSystem:d.bookingSystem}});
  ...
  <form ref={form} onChange={()=>{requestAnimationFrame(()=>setDetails(readDetails()));setSaved(false);setError("");}} onSubmit={submit} noValidate className={styles.form}>
   ...
   <section id="hours-team" tabIndex={-1} aria-labelledby="hours-title" data-step="2" className={styles.section}><h2 id="hours-title" className={styles.sectionTitle}>Hours & availability</h2>
    <div className={styles.columns}>
     <div className={styles.stack}>
      <section className={styles.card}><div className={styles.cardHeader}><div><h2>When you’re open</h2><p>When customers can book an appointment.</p></div><label className={styles.zone}>Time zone<select aria-label="Time zone" value={timezone} onChange={e=>setTimezone(e.target.value)}>{Array.from(new Set([...zones,timezone])).map(z=><option key={z} value={z}>{z.replace("America/","").replaceAll("_"," ")}</option>)}</select></label></div>
       ...
       <div className={styles.fields}><label>Opens<input type="time" required value={opens} onChange={e=>setOpens(e.target.value)}/></label><label>Closes<input type="time" required value={closes} onChange={e=>setCloses(e.target.value)}/></label></div><p className={styles.note}>These hours apply to every selected day.</p>
      </section>
     ...
     <aside className={`${styles.card} ${styles.week}`}><h2>Your week, at a glance</h2><p>Here’s when customers can book.</p><div className={styles.weekBars}>{[1,2,3,4,5,6,0].map(d=><div key={d}><span>{weekdays[d].slice(0,3)}</span><div data-open={days.includes(d)}>{days.includes(d)?<>{clock(opens)}<br/>–<br/>{clock(closes)}</>:"Closed"}</div></div>)}</div><div className={styles.booking}><p>Booking preview</p><div><span className={styles.avatar} aria-hidden="true">◷</span><div><strong>Appointment · {team[0]?.minutes??30} min</strong><p>with {details.business_name||"your business"}</p></div></div></div><p className={styles.note}>Booking rules and availability still apply.</p></aside>
    </div>
   ...
   <section id="review-setup" tabIndex={-1} aria-labelledby="review-title" data-step="3" className={styles.section}><h2 id="review-title" className={styles.sectionTitle}>Review & setup</h2><div className={styles.columns}>
    <div className={styles.stack}>
     ...
     <section className={styles.card}><div className={styles.cardHeader}><h2>Hours & availability</h2><button type="button" className={styles.edit} onClick={()=>move(2)}>Edit hours & team</button></div><p>{[1,2,3,4,5,6,0].filter(d=>days.includes(d)).map(d=>weekdays[d].slice(0,3)).join(", ")} · {clock(opens)}–{clock(closes)}</p><p>{timezone.replaceAll("_"," ")}</p><p>Appointment length: {team[0]?.minutes??30} minutes · One shared schedule</p></section>
