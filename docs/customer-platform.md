...
4. Create recurring Stripe prices: front $149/month, busy $299/month, full $599/month; setup $1,000 one time. Set `STRIPE_SECRET_KEY`, `STRIPE_PRICE_FRONT`, `STRIPE_PRICE_BUSY`, `STRIPE_PRICE_FULL`, and `STRIPE_PRICE_SETUP`. Start in Stripe test mode.
...
- Call usage currently counts completed customer confirmation calls by UTC calendar month. Additional-call amounts are estimates; automatic metered invoicing is not enabled. Review overage invoices manually during the concierge launch.
