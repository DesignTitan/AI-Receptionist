...
export function PurchaseWelcome({ name, plan, receipt, state, test = false }: {
  ...
  return <AccountShell billingAvailable={state !== "pending"}>
    ...
    <aside className={styles.support}><span className={styles.supportIcon} aria-hidden="true">↗</span><div><h2>Need a hand getting started?</h2><p>We’re here to help you set things up.</p></div><Link href="/#hear">Get help</Link></aside>
  </AccountShell>;
}
