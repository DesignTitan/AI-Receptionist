...
export function AccountSettings({name:initialName,email,plan,timezone,billingAvailable,contactEmail,preview=false}:{name:string;email:string;plan:Plan;timezone:string;billingAvailable:boolean;contactEmail?:string;preview?:boolean}) {
  ...
  return <AccountShell name={savedName} billingAvailable={billingAvailable} preview={preview}>
    <header className={styles.heading}><h1>Account settings</h1><p>Your profile, sign-in and subscription, in one place.</p></header>
    <div className={styles.layout}>
      <nav className={styles.sections} aria-label="Settings sections">{[["profile","Profile"],["security","Sign-in & security"],["billing","Billing"],["notifications","Notifications"],["privacy","Privacy & account"]].map(([id,label])=><a key={id} href={`#${id}`}>{label}</a>)}</nav>
      <div className={styles.panels}>
        ...
        <section id="security" className={styles.panel}><h2>Sign-in & security</h2><div className={styles.row}><div><h3>Secure email sign-in</h3><p>Sign in using the link sent to your email. There’s no password to remember.</p></div><span className={styles.badge}>Passwordless</span></div><div className={styles.row}><div><h3>This session</h3><p>Sign out of this browser when you’re finished.</p></div>{preview?<Link className={styles.secondary} href="/account/login">Go to sign-in</Link>:<RemoteAction url="/api/account/session" method="DELETE" label="Sign out" className={styles.secondary}/>}</div></section>
        <section id="billing" className={styles.panel}><h2>Billing</h2><div className={styles.row}><div><h3>{PLANS[plan].name}</h3><p>Review your subscription, invoices and payment details in the secure billing portal.</p></div>{billingAvailable?<RemoteAction url="/api/account/billing" label="Manage billing ↗" className={styles.secondary}/>:<span className={styles.badge}>{preview?"Example plan":"Available after checkout"}</span>}</div><p className={styles.hint}>For cancellation or changes unavailable in the portal, <a href={requestLink("Help with my subscription")}>contact support</a>.</p><div className={styles.row}><div><h3>Business time zone</h3><p>{timezone || "Set during business setup"}</p></div><a href={requestLink("Change my business time zone")}>Request a change</a></div></section>
