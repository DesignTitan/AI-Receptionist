...
export default function LegalPlaceholders() {
  return <main className={styles.page}>
    ...
    {LEGAL_GROUPS.map(group => <section key={group.title} id={`group-${group.title.split(" ")[0].toLowerCase()}`}><h2>{group.title}</h2><div className={styles.grid}>{group.items.map(item => <article id={item.id} key={item.id}><span className={styles.status}>Draft placeholder</span><h3>{item.title}</h3><p>{item.detail}</p>{item.id === "do-not-sell" && <p><a href="/do-not-sell-or-share-my-personal-information">Open the opt-out placeholder →</a></p>}<small>Final wording, applicability and supporting evidence: pending review.</small></article>)}</div></section>)}
